...
- `async reprocess_asset(self, asset_id: uuid.UUID) -> Asset`: Load asset. Re-run YOLO detection on its reference image (via to_thread). Re-run reverse-prompting. Update the following asset fields explicitly:
  - `reverse_prompt`: overwritten with new Gemini reverse-prompt output
  - `visual_description`: overwritten with new Gemini visual description
  - `quality_score`: overwritten with new Gemini quality score
  - `detection_class`: overwritten with new YOLO detection class
  - `detection_confidence`: overwritten with new YOLO confidence score
  - `is_face_crop`: re-evaluated based on new YOLO detection results
  - `crop_bbox`: overwritten with new YOLO bounding box coordinates
...
import inspect
# Verify all three key methods exist
methods = [m for m in dir(ManifestingEngine) if not m.startswith('_')]
print('assemble_contact_sheet' in methods, 'process_manifest' in methods, 'reprocess_asset' in methods)
"` -- should print True True True.

Run: `cd /home/ubuntu/work/video-pipeline && python -c "
# Test contact sheet generation with a real Pillow image
...
# Import and test assemble_contact_sheet directly
from vidpipe.services.manifesting_engine import ManifestingEngine

# Create a mock asset-like object with required attributes
class MockAsset:
    def __init__(self, name, asset_type, ref_path):
        self.name = name
        self.asset_type = asset_type
        self.reference_image_url = ref_path

mock_assets = [MockAsset('Test Char', 'CHARACTER', test_img_path)]
manifest_id = uuid.uuid4()

# Create output dir
out_dir = os.path.join(test_dir, 'manifests', str(manifest_id))
os.makedirs(out_dir, exist_ok=True)

# Call contact sheet assembly (engine uses tmp/ prefix, so we test the Pillow logic)
# We test that the method exists and Pillow works
from PIL import Image, ImageDraw, ImageFont
canvas = Image.new('RGB', (256, 316), 'white')
draw = ImageDraw.Draw(canvas)
thumb = Image.open(test_img_path)
thumb.thumbnail((256, 256))
canvas.paste(thumb, (0, 0))
draw.text((5, 261), '[1] Test Char', fill='black')
out_path = os.path.join(out_dir, 'contact_sheet.jpg')
canvas.save(out_path, quality=90)

# Verify output
assert os.path.exists(out_path), 'Contact sheet not created'
result = Image.open(out_path)
assert result.size[0] > 0 and result.size[1] > 0, 'Contact sheet is empty'
print(f'Contact sheet created: {result.size[0]}x{result.size[1]}px, {os.path.getsize(out_path)} bytes')

# Cleanup
import shutil
shutil.rmtree(test_dir)
"` -- should print contact sheet dimensions and size without errors.
  </verify>
  <done>ManifestingEngine class exists with process_manifest (full pipeline), assemble_contact_sheet (Pillow grid), and reprocess_asset methods. reprocess_asset explicitly updates reverse_prompt, visual_description, quality_score, detection_class, detection_confidence, is_face_crop, and crop_bbox. Progress tracking dict updated at each pipeline step. Contact sheet generation verified with Pillow.</done>
</task>

<task type="auto">
  <name>Task 2: Create background task runner and processing API endpoints</name>
  <files>backend/vidpipe/workers/__init__.py, backend/vidpipe/workers/processing_tasks.py, backend/vidpipe/api/routes.py, backend/vidpipe/services/manifest_service.py</files>
  <action>
...
- IMPORTANT: Use `async with async_session() as session:` pattern (not FastAPI's `Depends` session injection) because the ManifestingEngine needs a session it fully controls. Create a fresh session explicitly:
  ```python
  async with async_session() as session:
      engine = ManifestingEngine(session)
...
**4. Update backend `UpdateAssetRequest` Pydantic schema in `routes.py`:**

The existing `UpdateAssetRequest` schema only has: name, description, asset_type, user_tags, sort_order. Add two new optional fields so that Stage 3 inline editing of reverse_prompt and visual_description actually reaches the backend:
...
class UpdateAssetRequest(BaseModel):
    ...
    reverse_prompt: Optional[str] = None        # NEW: Stage 3 inline editing
    visual_description: Optional[str] = None     # NEW: Stage 3 inline editing
```

**5. Update `allowed_fields` in `manifest_service.update_asset()`:**

In `backend/vidpipe/services/manifest_service.py`, the `update_asset()` method has an `allowed_fields` set that gates which fields can be updated. Currently it is `{"name", "description", "user_tags", "sort_order"}`. Add the two new fields:

```python
allowed_fields = {"name", "description", "user_tags", "sort_order", "reverse_prompt", "visual_description"}
```

Without this change, even though the frontend sends reverse_prompt/visual_description in PUT requests, the backend will silently drop them.

**6. Update AssetResponse schema and _asset_to_response helper** to include the new Phase 5 fields:
...
Run: `cd /home/ubuntu/work/video-pipeline && python -c "
from vidpipe.api.routes import UpdateAssetRequest
fields = UpdateAssetRequest.model_fields
print('reverse_prompt' in fields, 'visual_description' in fields)
"` -- should print True True (verifies Stage 3 inline editing fields are accepted by backend).

Run: `cd /home/ubuntu/work/video-pipeline && python -c "
from vidpipe.services.manifest_service import ManifestService
import inspect
src = inspect.getsource(ManifestService.update_asset)
print('reverse_prompt' in src and 'visual_description' in src)
"` -- should print True (verifies allowed_fields includes both new fields).
  </verify>
  <done>Background task runner handles full manifesting pipeline with progress tracking. Three new API endpoints (process, progress, reprocess) are registered. AssetResponse includes Phase 5 fields. UpdateAssetRequest accepts reverse_prompt and visual_description for Stage 3 inline editing. manifest_service.update_asset() allowed_fields includes reverse_prompt and visual_description. Processing errors are caught and recorded.</done>
</task>
...
7. UpdateAssetRequest accepts reverse_prompt and visual_description fields
8. manifest_service.update_asset() allowed_fields includes reverse_prompt and visual_description
9. FastAPI server starts without import errors: `cd /home/ubuntu/work/video-pipeline && timeout 5 python -m vidpipe.api 2>&1 | head -20`
10. Contact sheet generation produces valid JPEG output (verified via Pillow test in Task 1)
</verification>

<success_criteria>
- Full manifesting pipeline can be triggered via API
- Progress is trackable via polling endpoint
- Single asset reprocessing works and updates all 7 fields (reverse_prompt, visual_description, quality_score, detection_class, detection_confidence, is_face_crop, crop_bbox)
...
- Stage 3 inline editing of reverse_prompt and visual_description works end-to-end (frontend PUT -> backend UpdateAssetRequest -> manifest_service.update_asset allowed_fields -> database)
</success_criteria>
