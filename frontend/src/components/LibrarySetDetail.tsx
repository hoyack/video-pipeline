...
import { IMAGE_MODELS } from "../lib/constants.ts";
...
function RefsTab({
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      await generateLibrarySetImage(libSet.id, prompt, selectedImageModel || undefined);
      onRefresh();
    ...
    <div className="space-y-4">
      ...
      {libSet.refs.length === 0 ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {libSet.refs.map((ref) => (
            <div key={ref.id} className="relative group rounded-lg border border-gray-700 overflow-hidden">
              <img
                src={ref.image_url}
