...
must_haves:
  truths:
    ...
    - "Crash-recovery resume for quality-mode projects uses multi-candidate poll and scoring, not standard-mode poll"
  artifacts:
...
**Step B2: Initialize `all_assets` before the manifest block to prevent NameError:**

In `_generate_video_for_scene`, BEFORE the existing `if project.manifest_id:` block (around line 540), add:
```python
all_assets: list = []
```

This ensures `all_assets` is always defined regardless of whether the project has a manifest. The existing `if project.manifest_id:` block will overwrite it with the real asset list when a manifest is present. Without this initialization, referencing `all_assets` when `manifest_id is None` raises `NameError`.
...
**Step G: Update crash-recovery resume path for quality mode:**

The existing crash-recovery block (around lines 655-671) unconditionally calls `_poll_video_operation` when a clip is in `status="polling"`. After this phase, a quality-mode project that crashes mid-Veo-operation would resume using the standard-mode poll which only reads `generated_videos[0]`, skipping multi-candidate collection, scoring, and auto-selection.

Modify the crash-recovery block to check for quality mode:
...
if clip and clip.status == "polling" and clip.operation_name:
    if project.quality_mode and project.candidate_count > 1:
        # Quality mode crash recovery: use multi-candidate poll
        poll_result, video_bytes_list = await _poll_and_collect_candidates(
            session, clip, client, project, scene, file_mgr, selected_refs,
        )
        if poll_result == "complete":
...
This ensures `all_assets` is safe to reference here because Step B2 initializes it to `[]` before this block, and the `if project.manifest_id:` block (which loads real assets) runs before the crash-recovery check.
...
4. Verify crash-recovery block branches on `project.quality_mode` — grep for `quality_mode` near the `status == "polling"` check to confirm the conditional exists
  </verify>
  <done>
video_gen.py supports multi-candidate generation: _submit_video_job passes number_of_videos, _poll_and_collect_candidates handles multi-video responses, _handle_quality_mode_candidates saves/scores/selects. Standard mode code path unchanged. Selected candidate's path goes to VideoClip.local_path. RAI filtering only escalates on zero survivors. Crash-recovery resume correctly uses multi-candidate poll and scoring for quality-mode projects. `all_assets` initialized to `[]` before manifest block to prevent NameError.
  </done>
</task>
...
5. Crash recovery: grep for `quality_mode` near `status == "polling"` — should show the branching conditional
6. all_assets init: grep for `all_assets: list = \[\]` or `all_assets = \[\]` — should appear before the `if project.manifest_id:` block
</verification>
...
- Crash-recovery for quality-mode projects resumes with _poll_and_collect_candidates and runs scoring/selection
- `all_assets` always defined (initialized to `[]`) so quality mode without manifest does not raise NameError
</success_criteria>
