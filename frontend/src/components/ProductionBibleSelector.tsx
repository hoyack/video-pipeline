...
import { getProductionBibleDetail, listProductionBibles } from "../api/client.ts";
import type { ProductionBibleDetail, ProductionBibleListItem } from "../api/types.ts";

interface ProductionBibleSelectorProps {
  selectedProductionBibleId: string | null;
  onProductionBibleSelect: (productionBibleId: string | null) => void;
}

export function ProductionBibleSelector({
  selectedProductionBibleId,
  onProductionBibleSelect,
}: ProductionBibleSelectorProps) {
  const [selectedDetail, setSelectedDetail] = useState<ProductionBibleDetail | null>(null);
  ...
  const filteredBibles = productionBibles.filter((m) =>
    m.name.toLowerCase().includes(search.toLowerCase()),
  );

  // Fetch detail when external selection changes (e.g. initial value)
  useEffect(() => {
    if (!selectedProductionBibleId) {
      setSelectedDetail(null);
      return;
    }
    async function fetchDetail() {
      try {
        const detail = await getProductionBibleDetail(selectedProductionBibleId!);
        setSelectedDetail(detail);
      } catch (err) {
        console.error("Failed to load production bible:", err);
        setSelectedDetail(null);
        onProductionBibleSelect(null);
      }
    }
    fetchDetail();
  }, [selectedProductionBibleId]);
  ...
  // Show selected production bible card
  if (selectedDetail) {
    return (
  ...
  if (!showPicker) {
    return (
      ...
      >
        ...
        Select a production bible to attach reference images
      </button>
    );
  }
        ...
        {fetchingList ? (
          ...
          filteredBibles.map((m) => (
            <button
              ...
              type="button"
              onClick={() => {
                onProductionBibleSelect(m.production_bible_id);
                setShowPicker(false);
              }}
